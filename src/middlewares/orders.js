...
const getOrders = async (req, res, next) => {
    ...
    orders.forEach(order => {
        ...
        if(!customers.includes(order.user_id)){
            indivOrders.push({user_id: order.user_id, name: order.cust_name, address: order.address, itemNum: 1, total: parseInt(order.total_payment), mode: order.mode_delivery, orders: [order]})
